...
aws s3 cp $BACKUP_FILE s3://$S3_BUCKET/airporttransfer/database-backup/

# Find and delete backups older than 30 days from S3
aws s3 ls s3://$S3_BUCKET/airporttransfer/database-backup/ | grep -Eo 'airporttransfer_[0-9]{4}-[0-9]{2}-[0-9]{2}.prod.backup.db' | while read -r line; do
    BACKUP_DATE=$(echo $line | grep -Eo '[0-9]{4}-[0-9]{2}-[0-9]{2}')
    if [[ $(date -d $BACKUP_DATE +%s) -lt $(date -d '30 days ago' +%s) ]]; then
        aws s3 rm s3://$S3_BUCKET/airporttransfer/database-backup/$line
    fi
done
